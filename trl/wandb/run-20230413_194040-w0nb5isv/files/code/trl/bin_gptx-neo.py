...
class RewardFn:
    ...
    def __call__(self, response_tensors, batch_query):
        ...
        if ("function_to_apply" in self._pipe_sent_kwargs and 
            self._pipe_sent_kwargs["function_to_apply"] == "none"
        ):
            labels_outputs = [output[1]["label"] for output in pipe_outputs]
            call_outputs   = [torch.tensor(output[1]["score"]) for output in pipe_outputs] 
        else:
            labels_outputs = [output["label"] for output in pipe_outputs]
            call_outputs   = [torch.tensor(output["score"]) for output in pipe_outputs]
        ...
        if self._verbose:
            ...
            for text, label, output in itertools.islice(
                zip(texts, labels_outputs, call_outputs), 5):

                table.add_row(str(text), label, str(output.item()))
