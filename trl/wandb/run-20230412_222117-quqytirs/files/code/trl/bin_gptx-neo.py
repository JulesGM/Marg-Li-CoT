...
def main(
    ...
    for epoch, batch in tqdm(
        ...
        rich.print(
            f"[bold blue]({epoch = } {lib_trl_utils.get_rank()} / {lib_trl_utils.get_world_size()})[/][white bold] " +
            f"average rewards: "
            f"{np.mean(list(all_rewards)):0.4} "
            f"+- {np.std(list(all_rewards)):0.1}"
        )

        if lib_trl_utils.get_rank() == "0":
            wandb.log({"avg_all_rewards": np.mean(all_rewards.tolist())})
