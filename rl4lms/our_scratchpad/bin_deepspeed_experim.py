...
NO_DEEPSPEED_PARALLELIZE = True
NO_DEEPSPEED_MODE = False
DTYPE = torch.bfloat16

APPROX_BATCH_SIZE = int(os.getenv("WORLD_SIZE", "0"))
...
MAX_ANSWER_LENGTH = None # 95th percentile. Answers are much longer than questions. we care about l**2
...
def main():
    ...
    if NO_DEEPSPEED_MODE:
        ...
        optimizers = {k: torch.optim.Adam(v.parameters(), lr=0.0001) for k, v in models.items()}
        ...
        for idx, (model_name, model) in enumerate(models.items()):
            ...
            if INFERENCE_MODE:
                ...
                engines[model_name], optimizers[model_name], _, _ = deepspeed.initialize(
                    model=model, 
                    # mp_size=world_size,
                    # replace_method="auto",
                    # replace_with_kernel_inject=True,
    ...
    def max_length_dataset(
        ...
        for entry in dataset:
            ...
            assert isinstance(question, (dict, transformers.tokenization_utils_base.BatchEncoding)), f"question is a {type(question)}"
            if max_length_question and len(question["input_ids"]) > max_length_question:
                continue
            
            answer = tokenizer(entry["answer"])
            assert isinstance(answer, (dict, transformers.tokenization_utils_base.BatchEncoding)), f"answer is a {type(answer)}"
    ...
    def max_squared_sum(
        ...
        for entry in dataset:
            ...
            if len(question["input_ids"]) ** 2 + len(answer["input_ids"]) ** 2 > max_squared_sum:
                continue
    ...
    def padder(batch):
        ...
        assert max_length_input_ids == max_length_attention_mask, (
            max_length_input_ids, max_length_attention_mask)
        ...
        assert len(tokens) == len(attention_mask), (len(tokens), len(attention_mask))
    ...
    assert MAX_SQUARED_SUM is None or (MAX_QUESTION_LENGTH is None and MAX_ANSWER_LENGTH is None), (
        MAX_SQUARED_SUM is None, (MAX_QUESTION_LENGTH is None, MAX_ANSWER_LENGTH is None))
    ...
    for input_ids, label_ids in tqdm(batches):
        if INFERENCE_MODE:
            ################################################################
            # Inference Test
            ################################################################
