...
class PPOOrchestrator(Orchestrator):
    ...
    def make_experience(self, num_rollouts: int = 1024, iter_count: int = 0):
        ...
        while len(ppo_rl_elements) < num_rollouts:
            ...
            if not self.rl_model.model.model.config.is_encoder_decoder:
                response_tensors = samples[:, query_tensors.shape[1]:]
            else:
                response_tensors = samples

            scores = torch.as_tensor(self.score(
                batch,
                question_ids=query_tensors,
                scratchpad_ids=response_tensors,
                meta_info=dict(mode="train", fn="make_experience"),
            ))
