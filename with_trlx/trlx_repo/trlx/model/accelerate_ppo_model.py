...
class AcceleratePPOModel(AccelerateRLModel):
    def __init__(self, *, base_model_class, config):
        ...
        assert all([x.device.type == "cuda" for x in self.model.parameters()]), (
            [x.device.type for x in self.model.parameters()]
        )

        self.store.clear_history()
        print("{config.method.target = }")
    ...
    def get_arch(self, config: TRLConfig):
        return GPTHydraHeadWithValueModel(
            model_base_class=self._base_model_class, 
            config=self.config.model.model_path, 
