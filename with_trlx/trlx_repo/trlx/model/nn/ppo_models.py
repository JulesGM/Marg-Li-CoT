...
class GPTHeadWithValueModel(nn.Module):
    ...
    def __init__(self,  *, model_base_class, config: Union[PretrainedConfig, str]):
        ...
        if isinstance(config, PretrainedConfig):
            self.model = model_base_class.from_config(config)
        else:
            self.model = model_base_class.from_pretrained(config)
    ...
    def forward(
        ...
        lm_logits = self.model.lm_head(hidden_states) # Works with seq 2 seq
...
class GPTHydraHeadWithValueModel(nn.Module):
    ...
    def __init__(
        self, *, model_base_class, config: Union[PretrainedConfig, str], num_layers_unfrozen: int = -1
    ):
        super().__init__()
        if isinstance(config, PretrainedConfig):
            self.model = model_base_class.from_config(config)
        ...
        if hasattr(self.model.config, "hidden_size"):
            self.n_embd = self.model.config.hidden_size
            self.model.config.n_embd = self.n_embd
        else:
            self.n_embd = self.model.config.n_embd
        ...
        if num_layers_unfrozen > 0:
            ...
            if not self.model.config.is_encoder_decoder:
                transformer_blocks = list(self.model.transformer.h)[-num_layers_unfrozen:]
                ...
                transformer_blocks = list(self.model.decoder.block)[-num_layers_unfrozen:]
                # Retrive hf_config to init
                hf_config = AutoConfig.from_pretrained(config)
                hf_config.n_embd = self.n_embd
                self.frozen_head = ModelBranch(
                    hf_config,
                    transformer_blocks,
                    self.model.decoder.final_layer_norm, #.. this is from t5 https://github.com/huggingface/transformers/blob/main/src/transformers/models/t5/modeling_t5.py#L850
                    self.model.lm_head,
                )


    def generate(self, input_ids, **x):
        # This is ok with seq 2 seq
        return self.model.generate(input_ids, **x)
