...
def pad_sequences(
    sequences, pad_side: str, pad_token_id: int
) -> torch.Tensor:
    assert isinstance(sequences, (list, tuple)), (
        f"sequences must be a list or tuple, got {type(sequences)}"
    )
        ...
        if pad_side == "left":
            if isinstance(s, torch.Tensor):
                padded_sequences.append(
                    F.pad(s, (max_len - len(s), 0), value=pad_token_id)
                )
            else:
                assert isinstance(s, (list, tuple)), type(s)
                padded_sequences.append([pad_token_id] * (max_len - len(s)) + s)
        elif pad_side == "right":
            if isinstance(s, torch.Tensor):
                padded_sequences.append(
                    F.pad(s, (0, max_len - len(s)), value=pad_token_id)
                )
            else:
                assert isinstance(s, (list, tuple)), type(s)
                padded_sequences.append(s + [pad_token_id] * (max_len - len(s)))
        else:
            raise ValueError(f"pad_side must be left or right, got {pad_side}")
...
class AccelerateRLModel(BaseRLModel):
    ...
    def __init__(self, config, train_mode=True):
        ...
        if config.model.tokenizer_path:
            assert "padding_side" in config.model.tokenizer_kwargs
            assert config.model.tokenizer_kwargs["padding_side"] == "left", (
                config.model.tokenizer_kwargs["padding_side"]
            )
            ...
            if not self.model.model.config.is_encoder_decoder:
                assert self.tokenizer.padding_side == "left", self.tokenizer.padding_side
        ...
        if hasattr(self.model.model, "gpt_neox"):
            assert False
            gpt_blocks = self.model.model.gpt_neox.layers
        elif not self.model.model.config.is_encoder_decoder:
            assert False
            gpt_blocks = self.model.model.transformer.h
        else:
            gpt_blocks = self.model.model.decoder.block
    ...
    def generate(self, input_ids, attention_mask=None, **kwargs):
        ...
        with torch.no_grad():
            ...
            if "eos_token_id" in kwargs:
                assert kwargs["eos_token_id"] == self.tokenizer.cls_token_id, (
                    kwargs["eos_token_id"]
                )
    ...
    def evaluate(self):
        """
        Samples model on `eval_prompts`, logs stats with 
        `reward_fn` or `metric_fn` if provided
        """
        stats = {}
        all_samples = []
        generate_time = time()
        ...
        for prompts in tqdm(self.eval_dataloader, desc="Evaluating"):
            ...
            ###################################################################
            # Jules code - Added code to extract questions and scratchpads
            ###################################################################
            question_tensors = prompts.input_ids
            scratchpad_tensors = samples[:, question_tensors.shape[1]:]
            questions.append(question_tensors)
            scratchpads.append(scratchpad_tensors)
            ###################################################################

        stats["generate_time"] = time() - generate_time

        ###################################################################
        # Jules code - Pad questions and answers
        ###################################################################
        question_ids = self.accelerator.gather(pad_sequences(
            questions, 
            pad_side="left", 
            pad_token_id=self.tokenizer.pad_token_id,
        ))
        scratchpad_ids = self.accelerator.gather(pad_sequences(
            scratchpads, 
            pad_side="right", 
            pad_token_id=self.tokenizer.pad_token_id,
        ))
        ...
        if self.accelerator.is_main_process:
            ...
            if self.reward_fn:
                rewards = torch.as_tensor(self.reward_fn(
                    samples,
                    question_ids=question_ids,
                    scratchpad_ids=scratchpad_ids,
                    meta_info=dict(mode="eval", fn="evaluate"),
                    ), dtype=torch.float)
    ...
    def learn(self):
        ...
        for i in range(self.config.train.epochs):
            rich.print(f"[bold blue]EPOCH {i} / {self.config.train.epochs} {self.n_updates_per_batch = } #####")
            for batch_idx, batch in enumerate(self.train_dataloader):
                print("batch_idx", batch_idx)
                print("batch shape", batch.query_tensors.shape)
        ...
        rich.print("we're out of the training loop")
