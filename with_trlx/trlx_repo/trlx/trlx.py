...
def train(
    *,
    ...
    """
    Dispatches online or offline reinforcement training depending on whether a 
    reward function or a list of samples & rewards is given
    ...
    """
    #######################################################################
    rich.print("[bold blue]`trlx.train()`")
    #######################################################################

    if reward_fn is not None:
        #######################################################################
        rich.print("[bold blue]`reward_fn is not None`")
        #######################################################################
        ...
        #######################################################################
        rich.print("[bold blue]`get_model`")
        #######################################################################
        
        model: AcceleratePPOModel = get_model(config.model.model_type
        )(base_model_class=model_base_class, config=config)
        assert model.tokenizer.padding_side == "left", model.tokenizer.padding_side
        ...
        #######################################################################
        rich.print("[bold blue]`make_experience`")
        #######################################################################
        ...
        if len(samples) != len(rewards):
            raise ValueError(
                f"Number of samples {len(samples)} should match the "
                f"number of rewards {len(rewards)}"
            )
        ...
        eval_pipeline = PromptPipeline(
            prompts=eval_prompts, tokenizer=model.tokenizer)
    ...
    rich.print("[bold blue]model.learn")
    model.learn()
