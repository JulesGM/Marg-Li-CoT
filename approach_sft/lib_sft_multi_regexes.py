...
class MultiFSMSequenceGenerator(
    ...
    def __call__(
        ...
        # print("-" * 80)
        # for str_ in self.tokenizer.tokenizer.batch_decode(
        ...
        if batch_size == 1 and num_samples == 1:
            return output[0][0], states
        elif batch_size == 1:
            return output[0], states
        elif num_samples == 1:
            return [samples[0] for samples in output], states
        else:
...
#              for regexes in regexes_str]
#             ), (
#             regexes_str)

#         fsms = []
#         for regex_str in regexes_str:
#             if regex_str in self._fsm_cache:
#                 new_fsm = self._fsm_cache[regex_str]
#             else:
...
#                 self._fsm_cache[regex_str] = new_fsm
